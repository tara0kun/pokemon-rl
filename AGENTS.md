...
**Codex Vision (VLM) Agent** (`generic_agent/`) に pivot。
...
| path | 用途 |
|------|------|
| `generic_agent/` | **現プロジェクト**。 VLM agent (Codex Opus/Haiku + screenshot + RAM bridge) |
...
| `manual.py` | C モード helper (Codex が brain 役の手動操作) |
| `smoke_test.py` | 動作確認 |
...
## モデル選択
- `Codex-opus-4-8` (`MODEL_BRAIN` in config.py): 高品質 Brain、 重要判断用
- `Codex-haiku-4-5` (`MODEL_RESCUE` in rescue_brain.py): cost-optimized、 navigate / rescue 用
