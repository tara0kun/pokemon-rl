...
def main():
    ...
    try:
        ...
        pass
    # ★ 2026-05-26: 追加 enforcement (RULE_VIOLATIONS #4/#6/#10 再発防止)
    # BestSave 6h check
    try:
        _bsm = "tools/.bestsave_marker"
        if _cos.path.exists(_bsm):
            _age_h = (_cad_now - _cos.path.getmtime(_bsm)) / 3600
            if _age_h >= 6:
                _cad_violations.append(
                    f"BestSave: 最終 {_age_h:.1f}h 前 (6h ルール違反)、 "
                    f"最進捗 instance を Slot 8 に save → `touch tools/.bestsave_marker`")
        else:
            _cad_violations.append(
                "BestSave: marker 不在、 一度も実施されていない可能性 "
                "(Slot 8 save 後 `touch tools/.bestsave_marker`)")
    except Exception:
        pass
    # MEMORY.md update 24h check (feedback memory 蓄積義務)
    try:
        _mem = _cos.path.expanduser(
            "~/.claude/projects/c--pokemon-rl/memory/MEMORY.md")
        if _cos.path.exists(_mem):
            _age_h = (_cad_now - _cos.path.getmtime(_mem)) / 3600
            if _age_h >= 24:
                _cad_violations.append(
                    f"MEMORY.md: {_age_h/24:.1f}d 未更新 "
                    f"(feedback memory 新規追加 or 既存更新検討)")
    except Exception:
        pass
    # Self-check 3 cycle/1 enforcement (marker counter)
    try:
        _scm = "tools/.selfcheck_marker"
        _last_cc = 0
        if _cos.path.exists(_scm):
            with open(_scm) as _f:
                _last_cc = int(_f.read().strip() or 0)
        # _cc 計算は後段だが先取り
        _cur_cc = 0
        if _cos.path.exists("tools/.monitor_calls"):
            with open("tools/.monitor_calls") as _f:
                _cur_cc = int(_f.read().strip() or 0)
        _delta = _cur_cc - _last_cc
        if _delta >= 3:
            _cad_violations.append(
                f"Self-check: {_delta} cycle 未実施 (3 cycle/1 回 ルール) "
                f"→ CLAUDE.md 全ルール再読 + 違反 audit → "
                f"`echo {_cur_cc} > tools/.selfcheck_marker`")
    except Exception:
        pass
    # AI work cycle-based (1 cycle/1 回 ideal、 5 cycle 連続 skip で違反)
    try:
        _awm = "tools/.ai_work_marker"
        _last_aw_cc = 0
        if _cos.path.exists("tools/.ai_work_cycle"):
            with open("tools/.ai_work_cycle") as _f:
                _last_aw_cc = int(_f.read().strip() or 0)
        _cur_cc2 = 0
        if _cos.path.exists("tools/.monitor_calls"):
            with open("tools/.monitor_calls") as _f:
                _cur_cc2 = int(_f.read().strip() or 0)
        _aw_delta = _cur_cc2 - _last_aw_cc
        if _aw_delta >= 5:
            _cad_violations.append(
                f"AI work: {_aw_delta} cycle 連続 skip (毎 cycle 1 実施推奨、 "
                f"5 cycle 違反) → tile_classifier eval/retrain or battle_ai "
                f"or map_analyzer 等 → `touch tools/.ai_work_marker && "
                f"echo {_cur_cc2} > tools/.ai_work_cycle`")
    except Exception:
        pass
    if _cad_violations:
        print()
