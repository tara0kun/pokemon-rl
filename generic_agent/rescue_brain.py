...
SYSTEM_PROMPT_NAVIGATE_LONG = (
    ...
    "3. If 'OSCILLATING' appears, you have been bouncing between two "
    "   maps. Stay on the CURRENT map. Pick a direction that explores "
    "   its interior tiles, NOT a border tile that would transition.\n"
    "4. If 'bfs_to_frontier=<dir>' appears AND <dir> not in blocked_here "
    "   AND <dir> not in AVOID, you MUST pick <dir>. This is the BFS-"
    "   verified first step toward unexplored territory.\n"
    "5. Only if bfs_to_frontier is absent, fall back to path_memory: "
    "   pick the 'first=<Word>' direction whose target map matches the "
    "   GOAL hint and whose first step is not blocked nor in AVOID.\n"
    "6. If none of the above apply on overworld, pick a non-blocked "
